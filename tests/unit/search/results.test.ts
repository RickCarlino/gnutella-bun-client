...
import { SearchService } from "../../../src/search/results";
...
test("search owns numbering, detached results, and event values", () => {
  ...
  const search = new SearchService((event) => events.push(event));
  ...
  search.clear();
  expect(() => search.resolve(1)).toThrow("no such result 1");
  search.ingest(origin, packet());
  expect(search.resolve(1).queryHops).toBe(2);
});
