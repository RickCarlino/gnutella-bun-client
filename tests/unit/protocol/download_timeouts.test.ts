...
import { connectDownloadSocket } from "../../../src/protocol/http_download_session";
import { makeNode, MockSocket, withTempDir } from "./node/helpers";
...
test("connect timeout identifies the endpoint and removes its timer", async () => {
  await withTempDir(async (dir) => {
    ...
    const connected = connectDownloadSocket(node, "127.0.0.1", 6346, {});
    socket.emit("timeout");
